...
import { X as CloseIcon } from "lucide-react"; // added close icon, assuming it might be missing from lucide imports

interface InlineDetailViewProps {
    item: Item & { folderKey?: number[] | null, rootFolderId?: string | null };
    onClose?: () => void;
    onDataRefresh?: () => void;
}

export function InlineDetailView({ item: initialItem, onClose, onDataRefresh }: InlineDetailViewProps) {
    const [item, setItem] = useState<(Item & { folderKey?: number[] | null, rootFolderId?: string | null }) | null>(initialItem);
        ...
        <div className="min-h-screen bg-slate-950 text-neutral-100 flex flex-col font-sans selection:bg-indigo-500/30 relative overflow-hidden">
            ...
            <header className="h-14 border-b border-white/5 bg-white/5 backdrop-blur-xl flex items-center justify-between px-6 sticky top-0 z-10 relative shadow-sm">
                ...
                <div className="flex gap-2 relative z-10 shrink-0 ml-3">
                    ...
                    {onClose && (
                        <button onClick={onClose} className="p-2 ml-2 hover:bg-white/10 rounded-lg text-white/50 hover:text-white transition-all" title="Close Panel">
                            <CloseIcon size={18} />
                        </button>
                    )}
                </div>
