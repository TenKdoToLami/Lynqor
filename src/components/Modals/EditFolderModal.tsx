...
export function EditFolderModal({
    ...
    return (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
            <div className="bg-slate-900/60 border border-white/10 rounded-3xl w-full max-md p-8 shadow-2xl backdrop-blur-2xl">
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-br from-white to-white/60">Edit Folder</h2>
                    </div>
                        ...
                        {encrypt && (
                            <div className="mt-4 pt-4 border-t border-white/10">
                                <label className="text-xs font-semibold text-indigo-300 mb-2 block ml-1 flex items-center gap-1">
                                    <Lock size={12} /> New Custom Password (Optional)
                                </label>
                                <input
                                    type="password"
                                    value={password}
                                    onChange={e => setPassword(e.target.value)}
                                    placeholder="Type new password, or leave blank to keep old / parent's"
                ...
                <div className="flex justify-end gap-3">
                    ...
                    <button onClick={onSave} className="px-6 py-3.5 rounded-2xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold">Save Changes</button>
                </div>
            </div>
        </div>
    );
